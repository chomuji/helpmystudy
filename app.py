...
def make_fill_in_blank(sent, used_words=None):
    if used_words is None:
        used_words = set()
    toks = tokenize(sent)
    candidates = [t for t in toks if t not in STOPWORDS and len(t) >= 3 and t not in used_words]
...
def generate_quiz(text, num_tf=3, num_blank=3, seed=None):
    ...
    # 빈칸 문제: 문장을 여러 번 돌면서 목표 개수 채우기
    while len(blank) < num_blank:
        for s in sents:
            if len(blank) >= num_blank:
                break
            q, ans = make_fill_in_blank(s, used_words)
            if q:
                blank.append({"question":q, "answer":ans})
        else:
            # 후보 단어가 다 소진되면 종료
            break
...
with st.form(key="quiz_form"):
    ...
    num_blank = st.number_input("빈칸 문제 개수", min_value=1, max_value=10, value=6)
